...
#   for step in 0..2047:
#      swap(this.memory[0+step*2], this.memory[1+step*2])
# 4096
# 2047
#   var kek: string
#   for i in 512..4095:
#     kek &= toHex(this.memory[i])
#   echo kek
  for line in 31..255:
    ram &= "\n" & toHex(line.uint16) & "\t"
    for first in 0..7:
      ram &= toHex(this.memory[line+first]) & " "
    ram &= "\t"
    for dol in 8..15:
      ram &= toHex(this.memory[line+dol]) & " "
...
proc fetch(this: var Cpu): uint16 =
    ...
    return (this.memory[this.PC].uint16 shl 8) or (this.memory[this.PC + 1].uint16 shl 0)
...
proc execute(this: var Cpu, instruction: (Instruction, seq[uint16]))=
    ...
    of "ADD_VX_VY":
        ...
        this.registers[0xf] = this.registers[args[0]] + (if this.registers[args[1]] > 0xff: 1 else: 0)
        this.registers[args[0]] += this.registers[args[1]]
        this.nextInstruction()
    of "SUB_VX_VY":
        # 8xy5 - Set Vx = Vx - Vy, set VF = NOT borrow
        this.registers[0xf] = (if this.registers[args[0]] > this.registers[args[1]]: 1 else : 0 )
        this.registers[args[0]] -= this.registers[args[1]]
        this.nextInstruction()
    of "SHR_VX_VY":
        # 8xy6 - Set Vx = Vx SHR 1
        this.registers[0xf] = this.registers[args[0]] and 1
        this.registers[args[0]] = this.registers[args[0]] shr 1
        this.nextInstruction()
    of "SUBN_VX_VY":
        # 8xy7 - Set Vx = Vy - Vx, set VF = NOT borrow
        this.registers[0xf] = if this.registers[args[1]] > this.registers[args[0]]: 1 else : 0
        this.registers[args[0]] = this.registers[args[1]] - this.registers[args[0]]
        this.nextInstruction()
    of "SHL_VX_VY":
        # 8xyE - Set Vx = Vx SHL 1
        this.registers[0xf] = this.registers[args[0]] shl 7
        this.registers[args[0]] = this.registers[args[0]] shl 1
        this.nextInstruction()
    of "SNE_VX_VY":
    ...
    of "DRW_VX_VY_N":
        ...
        for i in 0.uint16..(args[2]-1).uint16:
            ...
            for position in 0..7:
                ...
                let value = if (line and (1.uint8 shl (7 - position))) > 0: 1.uint8 else : 0.uint8
                # If this causes any pixels to be erased, VF is set to 1
                let x = (this.registers[args[0]] + position.uint8) mod DISPLAY_WIDTH # wrap around width
                let y = (this.registers[args[1]] + i) mod DISPLAY_HEIGHT # wrap around height
    ...
    of "SKP_VX": 
        ...
        if (this.`interface`.getKeys() and (1.uint16 shl this.registers[args[0]].uint16)) > 0:
            this.skipInstruction()
        else:
            this.nextInstruction()
    of "SKNP_VX":
